...
import { Play, Pause, XCircle, RefreshCw, Terminal, CheckCircle2, AlertTriangle, Loader2, ArrowLeft, Eye } from 'lucide-react'

export default function AgentRunView(): React.JSX.Element {
  const { activeJob, activeJobId, pauseJob, resumeJob, approveAndResumeJob, cancelJob, rerunJob, navigate } = useAppStore()
  const logEndRef = useRef<HTMLDivElement>(null)

  const formatCost = (input: number, output: number) => {
    const cost = (input * 0.0025 + output * 0.010) / 1000
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[400px] text-center space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Select a run from history or create a new script project to get started.</p>
        </div>
    ...
    <div className="w-full space-y-6 pb-12">
      ...
      <div className="glass-panel rounded-xl p-6 space-y-4 relative overflow-hidden">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex items-center space-x-2 shrink-0">
            {activeJob.status === 'running' && (
              <Button size="sm" variant="secondary" onClick={() => pauseJob(activeJob.jobId)} className="bg-white/10 hover:bg-white/20 border border-white/5">
            ...
            {activeJob.status === 'paused' && (
              <Button
                size="sm"
                onClick={() => {
                  const hasPendingAssets = activeJob.beats.some((b) => b.assets.some((a) => a.status === 'pending'))
                  if (hasPendingAssets) {
                ...
                className={activeJob.beats.some((b) => b.assets.some((a) => a.status === 'pending')) ? "bg-emerald-600 hover:bg-emerald-500 text-white border-none animate-pulse" : "bg-primary hover:bg-primary/95 text-white"}
              >
            ...
            {(activeJob.status === 'completed' || activeJob.status === 'failed' || activeJob.status === 'cancelled') && (
              <Button size="sm" onClick={() => rerunJob(activeJob.jobId)} className="bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white">
            ...
            {activeJob.status === 'completed' && (
              <Button size="sm" variant="secondary" onClick={handleInspectAssets} className="bg-emerald-600 hover:bg-emerald-500 text-white border-none">
        ...
        <div className="flex flex-wrap gap-6 text-xs text-muted-foreground font-mono pt-2 border-t border-white/5">
          ...
          <div>
            Downloads: <span className="text-emerald-400 font-semibold">{activeJob.downloadedCount} complete</span>
          </div>
          {activeJob.failedCount > 0 && (
            <div>
              Failed: <span className="text-red-400 font-semibold">{activeJob.failedCount} failed</span>
            </div>
          )}
          {activeJob.usage && activeJob.usage.totalTokens > 0 && (
            <div>
              Tokens: <span className="text-white font-medium">{activeJob.usage.inputTokens} in / {activeJob.usage.outputTokens} out</span>
              <span className="text-muted-foreground ml-1.5">(${formatCost(activeJob.usage.inputTokens, activeJob.usage.outputTokens)} est.)</span>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <span className="text-xs text-muted-foreground mt-0.5 block leading-normal">
                {activeJob.logs.find(l => l.type === 'error')?.message || 'The agent execution failed due to an error. Check the logs console for technical details.'}
              </span>
            </div>
          </div>
          <Button size="sm" onClick={() => rerunJob(activeJob.jobId)} className="bg-red-600 hover:bg-red-500 text-white font-medium border-none shrink-0 ml-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center space-x-2 pb-2">
            ...
            <Badge variant="secondary" className="bg-white/5 border border-white/5 text-muted-foreground">
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex items-center space-x-3">
                      ...
                      <h3 className="font-semibold text-sm capitalize">{beat.id.replace('_', ' ')}</h3>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <div className="text-sm font-medium leading-relaxed select-all">"{beat.text}"</div>
                    <div className="text-xs text-muted-foreground bg-black/10 border border-white/5 rounded p-2.5 font-sans leading-relaxed select-all">
                      <span className="font-mono text-[10px] text-primary uppercase block mb-1">Visual Direction:</span>
                      {beat.visualPrompt}
                    </div>
                  </div>
                  ...
                  {beat.searchQueries.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 items-center">
                      <span className="text-[10px] font-mono text-muted-foreground uppercase mr-1">Searches:</span>
                      {beat.searchQueries.map((query) => (
                        <Badge key={query} variant="outline" className="bg-black/30 text-white border-white/10 text-[10px]">
                          {query}
                        </Badge>
                      ))}
                    </div>
                  ...
                  {beat.assets && beat.assets.length > 0 && (
                    <div className="border-t border-white/5 pt-3 mt-3">
                      <div className="text-[10px] font-mono text-muted-foreground uppercase mb-2">Beat Stock Assets:</div>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {beat.assets.map((asset) => (
                          <div key={asset.id} className="relative aspect-video rounded-lg overflow-hidden border border-white/5 bg-neutral-900 group">
                            {asset.type === 'video' ? (
                              <img src={asset.imageUrl} className="w-full h-full object-cover" alt="Video Thumbnail" />
                            ) : (
                              <img src={asset.imageUrl} className="w-full h-full object-cover" alt="Stock Photo" />
                            )}
                            <div className="absolute inset-0 bg-black/40 flex flex-col justify-end p-2 opacity-100 transition-opacity">
                              <span className="text-[9px] font-semibold text-white block capitalize">{asset.type}</span>
                              <div className="flex items-center justify-between mt-1 text-[8px] font-mono">
                                <span className="text-neutral-300 truncate max-w-[80px]">{asset.photographer}</span>
                                {asset.status === 'completed' && <span className="text-emerald-400">Complete</span>}
                                {asset.status === 'downloading' && <span className="text-blue-400">Downloading ({asset.progress || 0}%)</span>}
                                {asset.status === 'failed' && <span className="text-red-400">Failed</span>}
                                {asset.status === 'pending' && <span className="text-neutral-400">Queued</span>}
                              </div>
                            </div>
                          </div>
                        ))}
                  ...
                  {beat.rejectedAssets && beat.rejectedAssets.length > 0 && (
                    <div className="border-t border-white/5 pt-3 mt-3">
                      <div className="text-[10px] font-mono text-muted-foreground uppercase mb-2">Skipped / Rejected Assets:</div>
                      <div className="space-y-1.5">
                        {beat.rejectedAssets.map((rej, idx) => (
                          <div key={idx} className="flex items-center justify-between text-xs bg-red-950/15 border border-red-500/10 rounded p-2 text-red-300">
